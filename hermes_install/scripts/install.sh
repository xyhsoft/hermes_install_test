...
install_system_deps() {
    ...
    if command -v python3 &>/dev/null; then
        ...
        if [[ "$need_brew_python" == "true" ]]; then
            ...
            if ! ver_ge "$py_version" "3.11" || ver_ge "$py_major_minor" "3.14"; then
                error "Python 版本 $py_version 不在 hermes-agent 支持范围（需要 >= 3.11 且 < 3.14）"
